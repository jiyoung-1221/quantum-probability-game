        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 whitespace-pre-line text-sm leading-7 text-slate-300 sm:text-base">
              {`양자역학에서 ‘확률’은 어떤 의미를 가질까요?

4개의 탐험 미션을 통해
양자역학 속 확률의 의미를 알아봅시다.`}
            </p>
          </div>
          <div className="rounded-lg border border-white/10 bg-white/5 p-4 lg:w-72">
            ...
            <p className="mt-3 text-sm text-slate-300">
              {completedCount}/{concepts.length}개 탐험 완료
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {concepts.map((concept) => {
          ...
          return (
            <button
              className={`group relative flex min-h-[31rem] flex-col overflow-hidden rounded-lg border bg-slate-950/72 p-5 text-left shadow-2xl backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-950 ${tone.card} ${tone.ring}`}
              key={concept.id}
            ...
            >
              ...
              <div className="relative mt-5 min-h-24">
                <h3 className="text-xl font-bold leading-8 text-white">
                  ...
                  <span className="mt-1 block text-lg font-semibold leading-7 text-slate-200">
                    {concept.subtitle}
                  </span>
                </h3>
              </div>

              <p className="relative min-h-28 text-sm leading-6 text-slate-300">
                {concept.summary}
              </p>

              <div className="relative mt-5 min-h-32 rounded-lg border border-white/10 bg-white/5 p-3">
